...
%       within 1e-7 and less from each other.
    ...
    function dir = isometricTransp(y, x, d)
        v = x-y-y*((x-y).'*y);
        v = M.dist(y,x)/norm(v, 'fro') * v;
        normV = norm(v, 'fro');
        u = v/normV;
        dir = d + (cos(normV)-1)*(u.'*d)*u-sin(normV)*(u.'*d)*y;
    end

    M.isotranspdir = @(x,alpha,p,d) isometricTranspDir(x,alpha,p,d);
    function dir = isometricTranspDir(x,alpha,p,d)
        u = alpha * p;
        normVel = norm(u, 'fro');
        u = u/normVel;
        dir = d + (cos(normVel)-1)*(u.'*d)*u - sin(normVel)*(u.'*d)*x;
    end
end
