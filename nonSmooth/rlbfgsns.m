function [x, cost, info, options] = rlbfgsns(problem, x0, options)
...
%         step = M.lincomb(xCur, alpha, p);
        ...
        [xNext, costNext, alpha, fail, lsiters] = linesearchnonsmooth(problem, M, xCur, p, xCurCost, M.inner(xCur,xCurGradient,p));
        stepsize = alpha * M.norm(xCur, p);
        step = M.lincomb(xCur, alpha, p);
...
function [xNext, costNext, t, fail, lsiters] = linesearchnonsmooth(problem, M, xCur, d, f0, df0)
    c1 = 0.01;
    c2 = 0.7;
    max_counter = 50;
