...
dim_set = [500]; %dimension of the Adjacency Matrix
...
for repeat = 1 : n_repeat
    ...
    for dim = dim_set
        ...
        for density = density_set
            ...
            result = clientconstrained_BC_ALM_compare(L, rank, options, specifier);
            result = result(:);
            param = [dim; density; repeat];
            outputdata = [result; param]';
            
            filename = sprintf('zz_BC_ALMbdd_Dim%d.dat', dim);
